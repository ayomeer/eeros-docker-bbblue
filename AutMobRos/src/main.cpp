...
int main(int argc, char **argv)
{
    ...
    // UNCOMMENT TO ACCESS HARDWARE THROUGH HAL
    log.info() << "Initializing hardware...";
    eeros::hal::HAL& hal = eeros::hal::HAL::instance();
    hal.readConfigFromFile(&argc, argv);

    log.info() << "Initializing control system...";
    ControlSystem cs(dt); // Create ControlSystem instance

    log.info() << "Initializing safety system...";
    MyRobotSafetyProperties sp(cs, dt);
    eeros::safety::SafetySystem ss(sp, dt);
    cs.timedomain.registerSafetyEvent(ss, sp.seSystemOff); // fired if timedomain fails to run properly
